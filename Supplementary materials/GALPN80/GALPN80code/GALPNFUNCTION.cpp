...
void FindBestIndividual(void)
{
	...
	for (i = 0; i<POPSIZE; i++)
	{
		t = abs((int)(SAMPLESIZE*(1 - PROBABILITY) - population[i].fitness));
		if (t <= BESTUPDOWN)
		{
			for (j = 0; j<LENGTH; j++)
			{
				if (s_1[j] != population[i].chorm[j])
					break;
			}

			if (j == LENGTH)
			{
				printf("The index of the current best individual %d. The fitness value is %d. The current individual is the optimal solution s*********** successful \n", i, population[i].fitness);
				fprintf(fpout, "The index of the current best individual %d. The fitness value is %d. The current individual is the optimal solution s*********** successful \n", i, population[i].fitness);
				for (k = 0; k<LENGTH; k++)
				{
					printf("%d ", population[i].chorm[k]);
				}
				printf("\n");
				exit(0);
			}
		...
		if (205<= population[i].fitness && population[i].fitness <=260)
		{
			for (k = 0; k < LENGTH; k++)
			{
